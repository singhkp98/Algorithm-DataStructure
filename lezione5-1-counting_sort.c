...
void counting_sort(int* A, int* B, int* C, int n, int k) {
  ...
  for (int i=0;i<=k;i++){ /// reset array conteggi li metto tutti a zero
    C[i]=0;
    ct_opw++;
  }
  ...
  for (int i=1;i<=k;i++){ /// C[i] contiene il numero di elementi <= i, Quali elementi sono minori o uguali all'indice i
    //calcolo la posizione di ciascun elemento nell'array 
    C[i]+=C[i-1];
    ct_opw++;
  }
  ...
  //posiziono l'elemento nella posizione corretta e decremento
