...
// 2) analisi con grafo (insert front, front + tail)
// ./a.out -graph

// 3) analisi con grafo (insert after elem)
// ./a.out -graph

// 4) analisi con grafo (delete front, delete tail, delete node)
// ./a.out -graph

// 5) implementa print reverse (ricorsivo)

// 6) implementa delete list (con rimozione esplicita di tutti elementi)

// 7) implementa double linked list e inserimento in testa
...
void list_delete_node(list_t *l, int search_elem){
  ...
  /// caso generale
    ...
    if (current_node->next->val == search_elem){ /// se il nodo next e' il nodo che cerco, lo cancello
      ...
      if (graph) print_status(l,node_del,"DEL NODE: connetto next del current al nodo successivo ");

      node_del->next=NULL;
      
      if (graph) print_status(l,node_del,"DEL NODE: sgancio next del nodo di interesse");
