...
//
// Obiettivo:
// 1) euler tour: stampa e ricostruzione albero da lista nodi
// 2) delete albero
...
node_t* build_euler(node_t* nodo){
  ...
  if(v!=nodo->val){
  	...
  	build_euler(nodoL);
  	input_visit >> v;
  	printf("Letto il valore %d dopo la visita ricorsiva L\n", v);
  }
  ...
  if(v!=nodo->val){
  	...
  	build_euler(nodoR);
  	input_visit >> v;
    printf("Letto il valore %d dopo la visita ricorsiva R\n", v);
  }
...
int main(int argc, char **argv) {
  ...
  */
  ...
  input_visit >> val;
  printf("root %d\n",val);
  node_t* root = node_new(val);
  build_euler(root); //equivale a crea()
  global_ptr_ref=root;
  input_visit.close();
  tree_print_graph(root);
  n_operazione++;
