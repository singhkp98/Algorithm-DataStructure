...
// 1) parsing espressione aritmetica in notazione polacca inversa (./a.out "1 2 + 5 * 2/")
// 2) analisi elaborazione su stack durante parsing
...
int eval(my_stack* s){ 
  ...
  if (op==1){ /// somma
    val1=eval(s);
    val2=eval(s);
    stack_push(s,val1+val2,0);
  }
  if (op==2){ /// -
    val1=eval(s);
    val2=eval(s);
    stack_push(s,val2-val1,0);
  }
  if (op==3){ /// *
    val1=eval(s);
    val2=eval(s);
...
int main(int argc, char **argv) {
  ...
  while (arg_ct<argc){ /// stringhe non terminate
    ...
    //// inserisco la valutazione (sequenziale!)
    int val,op;
    stack_top(s,val,op);
    if (op>0) /// c'e' un'operazione -> valuto e aggiorno stack
      eval(s);
