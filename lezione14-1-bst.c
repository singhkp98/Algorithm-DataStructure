...
//
// Obiettivo:
// 1) ricerca di un valore in un bst
...
int main(int argc, char **argv) {
  ...
  tree_insert_child_L(root->L,7);
  tree_insert_child_R(root->L,23);
  tree_insert_child_R(root,40);
  tree_insert_child_L(root->R,37);
  tree_insert_child_R(root->R,50);
