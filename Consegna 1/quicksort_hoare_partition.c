...
// compilazione: g++ -xc++ quicksort_hoare_partition.c 
//
// Obiettivo:

// prove con array ordinato decrescente --> numero di confronti?
// ./a.out 1024 -d=16
...
int partition(int* A, int p, int r){
  ...
  int i=p-1;
  int j=r+1;
  //cerco l'elemento più a sinistra maggiore o uguale al pivot
  while(1){
    do{
      i++;
    }
    ...
    //cerco l'elemento più a destra minore o uguale al pivot
    do{
      j--;
    }
    while(A[j]>x);
    
    //se i due indici si incontrano
    if(i>=j)
      return j;
...
int main(int argc, char **argv) {
  ...
  for (n = max_dim/ndiv;n<=max_dim;n+=max_dim/ndiv){
    ...
    if (ndiv>1)
      printf("%d,%d,%d,%.1f,%d,%.1f,%d,%.1f,%d,%.1f,%.1f\n",
	     n,ntests,
	     swap_min,(0.0+swap_avg)/ntests,swap_max,0.0+n*n,
	     cmp_min ,(0.0+cmp_avg)/ntests ,cmp_max ,0.0+n*n,0.0+n*log(n)/log(2));
  }
