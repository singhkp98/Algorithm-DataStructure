...
// compilazione: g++ -xc++ lezione8-1-list.c 
//
// Obiettivo:
...
//nodo che viene passato per parametro)
...
void list_print_reversed(node_t* n){
	if (n != NULL) {
        list_print_reversed(n->next);
        printf("%d, ", n->val);
    }
}
...
int main(int argc, char **argv) {
  ...
  printf("stampa lista in reverse \n");
  list_print_reversed(list->head);
