...
// compilazione: g++ -xc++ lezione10-5-stack-queue-dot.c 
//
// Obiettivo:
...
//restituisce il primo elemento (equivale alla top dello stack)
...
int main(int argc, char **argv) {
  ...
  //funzione che prende i caratteri della stringa e li butta nella coda 
  if (argc==2){
    ...
    for (int i=0;i<strlen(argv[1]);i++)
      stack_push(s,argv[1][i]);

    //printf("%d\n",queue_dequeue(q));

    for (int i=0;i<strlen(argv[1]);i++){
      if(queue_dequeue(q)==stack_pop(s))
        count++;
    }
