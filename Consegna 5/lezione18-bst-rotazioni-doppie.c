...
// compilazione: g++ -xc++ lezione18-bst-rotazioni-doppie.c -o avl
//
// Obiettivo:
...
void doubleRotateRight(node_t*& T){
  singleRotateLeft(T->L);
  singleRotateRight(T);
}
...
void rotateRight(node_t*& T){
  printf("Rotate Right: root %d\n",T->val);
  node_t* l = T->L;
  printf("Rotate Right: root->L %d\n",l->val);

  int  zig = get_height(l->L);
  int  zag = get_height(l->R);
  printf("confronto zag %d, zig %d\n",zag,zig);
  if(zig > zag)
    singleRotateRight(T);
  else
    doubleRotateRight(T);
...
int bilanciato(node_t* nodo){
  ...
  //controllo se è bilanciato a destra e sinistra e se la differenza fra le due altezze è <=1  
  return bl & br & abs(l-r)<=1;  
}

int profondita_log(node_t* nodo, int n){
	if(nodo==NULL){
		return 1; //vero
	}
	int alt = altezza(nodo); 
	int value = log(n)/log(2);
	if(alt==value)
		printf("L'albero ha profondita' logaritmica \n");
	else
		printf("L'albero non ha profondita' logaritmica \n");
}
...
int main(int argc, char **argv) {
  ...
  node_t* result=find(root,60);

  if(result==NULL)
  	printf("Il valore ricercato non esiste \n");
  else
  	printf("Il valore ricercato e' stato trovato \n");

  int a=altezza(root);
  printf("L'albero ha altezza %d\n",a);
  
  printf("L'albero e' bilanciato? %d\n",bilanciato(root));
  printf("L'albero ha profondita' logaritmica? %d\n",profondita_log(root,13));
